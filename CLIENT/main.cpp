	...
	case WM_PAINT: {
		if (my_info.id != -1) {
			...
			for (const auto& p : players) {
				...
				else other_ch_img.Draw(mdc, ((10 - (my_info.x - p.second.x)) * (TILE_SIZE)) + 5, ((10 - (my_info.y - p.second.y)) * (TILE_SIZE)) + 5, TILE_SIZE - 10, TILE_SIZE - 10, (my_motion / 5) * 28, int(p.second.dir) * 28, 28, 28);
			}
