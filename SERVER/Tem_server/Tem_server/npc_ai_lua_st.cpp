...
void process_packet(int c_id, char* packet)
{
	...
	case CS_LOGIN: {
		...
		_st_lock[clients[c_id].y / S_HEIGHT][clients[c_id].x / S_WIDTH].lock();
		sector_list[clients[c_id].y / S_HEIGHT][clients[c_id].x / S_WIDTH].insert(c_id);
		_st_lock[clients[c_id].y / S_HEIGHT][clients[c_id].x / S_WIDTH].unlock();
	...
	case CS_MOVE: {
		...
		if (s_x != clients[c_id].x / S_WIDTH || s_y != clients[c_id].y / S_HEIGHT) {
			_st_lock[clients[c_id].y / S_HEIGHT][clients[c_id].x / S_WIDTH].lock();
			sector_list[clients[c_id].y / S_HEIGHT][clients[c_id].x / S_WIDTH].erase(c_id);
			_st_lock[clients[c_id].y / S_HEIGHT][clients[c_id].x / S_WIDTH].unlock();

			_st_lock[s_y][s_x].lock();
			sector_list[s_y][s_x].insert(c_id);
...
void do_npc_random_move(int npc_id)
{
	...
	for (int y = s_y - 1; y < s_y + 2; ++y) {
		for (int x = s_x - 1; x < s_x + 2; ++x) {
			if (y < 0 || y >= W_HEIGHT / S_HEIGHT || x < 0 || x >= W_WIDTH / S_WIDTH) continue;
			_st_lock[y][x].lock();
			for (auto& j : sector_list[y][x]) {
				if (ST_INGAME != clients[j]._state) continue;
				if (true == is_npc(clients[j]._id)) continue;
				if (true == can_see(npc._id, clients[j]._id))
					old_vl.insert(clients[j]._id);
			}
			_st_lock[y][x].unlock();
		}
	}
	...
	if (s_x != npc.x / S_WIDTH || s_y != npc.y / S_HEIGHT) {
		_st_lock[npc.y / S_HEIGHT][npc.x / S_WIDTH].lock();
		sector_list[npc.y / S_HEIGHT][npc.x / S_WIDTH].erase(npc_id);
		_st_lock[npc.y / S_HEIGHT][npc.x / S_WIDTH].unlock();

		_st_lock[s_y][s_x].lock();
		sector_list[s_y][s_x].insert(npc_id);
		_st_lock[s_y][s_x].unlock();
	}
				...
				for (int x = s_x - 1; x < s_x + 2; ++x) {
					...
					for (auto& j : sector_list[y][x]) {
						...
						if (can_see(static_cast<int>(key), j)) {
							...
							ex_over->_ai_target_obj = j;
							clients[key]._ll.lock();
							auto L2 = clients[key]._L;
							lua_getglobal(L2, "event_bye_move");
							lua_pushnumber(L2, ex_over->_ai_target_obj);
							lua_pcall(L2, 1, 0, 0);
...
int API_check_move_start_time(lua_State* L)
{
	int user_id = (int)lua_tointeger(L, -1);
	lua_pop(L, 2);
	clients[user_id].move_start_time = chrono::system_clock::now();
	clients[user_id].hello = true;
	return 1;
}

int API_SendMessgeBye(lua_State* L)
{
	int npc_id = (int)lua_tointeger(L, -1);
	lua_pop(L, 2);
	bool x = false;
	if (clients[npc_id].hello && clients[npc_id].move_start_time + 3s <= chrono::system_clock::now()) {
		x = true;
		clients[npc_id].hello = false;
	}
	return x;
}
...
void InitializeNPC()
{
	...
	for (int i = MAX_USER; i < MAX_USER + MAX_NPC; ++i) {
		...
		clients[i]._id = i;
		clients[i].hello = false;
		sprintf_s(clients[i]._name, "NPC%d", i);
		clients[i]._state = ST_INGAME;

		_st_lock[clients[i].y / S_HEIGHT][clients[i].x / S_WIDTH].lock();
		sector_list[clients[i].y / S_HEIGHT][clients[i].x / S_WIDTH].insert(i);
		_st_lock[clients[i].y / S_HEIGHT][clients[i].x / S_WIDTH].unlock();
